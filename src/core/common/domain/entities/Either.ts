...
interface Left<L> { kind: 'left', leftValue: L }
interface Right<R> { kind: 'right', rightValue: R }
...
export class Either<L, R> {
  ...
  getOrThrow(errorMessage?: string): R {
    const throwFn = () => {
      throw new Error(
        errorMessage || `An error has ocurred retrieving value: ${JSON.stringify(this.value)}`,
      )
    }
